...
6) Обработка массовых запросов (5000+ ID в одном запросе) - Ассинхронная обработка. Клиент получает 202 Accepted и
   taskId, а сама обработка уходит в очередь (Kafka). Результаты можно показывать в реальном времени с помощью
   WebSockets
7) Как бы вы вынесли реестр утверждений в отдельную систему - выделение registry-service. В основном сервисе при апруве
   создается событие, которое через брокер сообщений попадает в сервис реестра для фиксации записи.